...
class TelegramClient:
    def __init__(self, settings: Settings) -> None:
        ...
        self.error_chat_id = settings.telegram_error_chat_id
        self.http = httpx.AsyncClient(timeout=30)

    async def aclose(self) -> None:
        await self.http.aclose()

    async def send(self, text: str, *, error_channel: bool = False) -> None:
        token = self.error_token if error_channel else self.main_token
        chat_id = self.error_chat_id if error_channel else self.main_chat_id
